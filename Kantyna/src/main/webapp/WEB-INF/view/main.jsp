...
<head>
	...
	function usuwanie(id)
	{
		...
		if(result.value)
		{
			...
			//var formularze = document.getElementsByClassName("zmienStatus");
...
<body style="background-color:#d1e1bf">
	...
	<sec:authorize access="hasRole('KLIENT')">
		...
		<div align="center">
			<font size="6">
			<strong>
				<c:if test="${not parametry.czyZamkniete}">
					<s:message code="page.main.GodzinyPracy"/>
					<c:out value="${parametry.godzinaOtwarcia}"/> - <c:out value="${parametry.godzinaZamkniecia}"/>
				</c:if>
				<c:if test="${parametry.czyZamkniete}">
		...
		<div class="form-group row" style="width: 100%;">
			...
			<div class="col-3">
				<input id="nazwaPotrawy" type="text" class="form-control" onkeyup="szukajPoNazwie()" name="nazwa" placeholder="nazwa"/>
			</div>
		</div>
			...
			<tr>
				...
				<td>
					<c:if test="${potrawa.czyPromocja}">
						<c:out value="${potrawa.cenaPromocyjna}" />
					</c:if>
				...
				<td>
					...
					<c:choose>
						...
						<c:otherwise>
							<button id="koment" type="button" class="btn btn-primary" data-toggle="modal" data-target="#komentarze" onclick="wyswietlKomentarze(${potrawa.id})">
								<s:message code="page.main.Komentarze2"/>
							</button>
						</c:otherwise>
					</c:choose>
	...
	<script>
		...
		$(document).ready(function () 
		{
			var ileStron = Math.ceil(${iloscRekordow}/5);
			...
			for(i = 0; i < ileStron; i++)
			{
				...
				btn.id = i + 1;
				//btn.setAttribute("id", "1"); //lub to jesli powyzsze nie zadziala
		...
		function wyswietlRekordy()
		{
    		var table, tr, td, i, txtValue;
		 	...
		 	var ktoryButton = event.srcElement.id
	  		for (i = 0; i < tr.length; i++) 
	  		{
		    	...
		    	else if( (i >= (((ktoryButton - 1) * 5) + 1)) & (i <= (((ktoryButton - 1) * 5) + 5)) ) //pierwszy rzad to naglowek, wiec wlasciwe rzedy zaczynaja sie od 1
			    {
			    	tr[i].style.display = "";
			    }
		        else 
...
<!--  	<div>
			<form:input id="numerPotraw" type="number" path="numerPotrawy" value="${}" style="display: none"/>
		</div> -->
			...
			<script>
				...
				//szukanie na podstawie wpisu w polu tekstowym
				function szukajPoNazwie()
				{
					//input = document.getElementById("nazwaPotrawy"); //to jest jquery, ponizej javascriptowy odpowiednik
			        var wartosc = $("#nazwaPotrawy").val();
	        		var table, tr, td, i, txtValue;
				  	table = document.getElementById("potrawy");
				 	tr = table.getElementsByTagName("tr");
				  	// Loop through all table rows, and hide those who don't match the search query
				  	if(wartosc != null)
				  	{
				  		for (i = 0; i < tr.length; i++) 
					  	{
						    ...
						    {
						      	txtValue = td.textContent || td.innerText;
							    if(txtValue.toLowerCase().includes(wartosc.toLowerCase()))
							    {
							    	tr[i].style.display = "";
							    }
						        else 
			...
			<div class="modal fade" id="komentarz1" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
					<form:form action="" method="POST" modelAttribute="Komentarz">
